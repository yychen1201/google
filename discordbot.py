...
@client.event                             
async def on_ready():                  
    print(f'登入：{client.user}') 
    discord.Activity(type=nextcord.ActivityType.playing, name="打/help領指令清單")
        await client.change_presence(activity=activity_w)   
        await asyncio.sleep(5)
        activity_w = discord.Activity(type=nextcord.ActivityType.playing, name="正在服務YT陳宥輔")
        await client.change_presence(activity=activity_w)
        await asyncio.sleep(5)
